...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    override public func draw (_ dirtyRect: NSRect) {
        ...
        // While an IME composition is in flight, open a visual gap at the caret so
        // the uncommitted text does not paint over the characters to its right.
        // This is purely a rendering effect: nothing is sent to the pty, and
        // committing/cancelling the composition clears it automatically.
        if let imeGap = imeCompositionGap() {
            drawWithIMEGap(dirtyRect: dirtyRect, context: currentContext, gap: imeGap)
        } else {
            drawTerminalContents (dirtyRect: dirtyRect, context: currentContext, bufferOffset: terminal.displayBuffer.yDisp)
        }
    }

    // MARK: - IME composition gap

    /// Geometry (view coordinates) of the gap opened for the in-flight IME
    /// composition on the current frame.
    private struct IMECompositionGap {
        let rowY: CGFloat       // bottom edge of the composing row
        let rowHeight: CGFloat
        let gapX: CGFloat       // left edge of the gap (caret cell origin x)
        let gapWidth: CGFloat   // width of the gap, in points
    }

    /// Total column width of the current marked (composing) text, measured with
    /// the terminal's own width table so CJK counts as 2 and combining marks as 0.
    private func imeMarkedColumns() -> Int {
        guard let composing = markedTextStorage?.string else { return 0 }
        var cols = 0
        for scalar in composing.unicodeScalars {
    ...
    /// Returns the gap geometry when an IME composition is active with a non-zero
    /// column width; otherwise nil, meaning the frame renders normally.
    private func imeCompositionGap() -> IMECompositionGap? {
        guard hasMarkedText(), let caretView, cellDimension != nil else { return nil }
        let cols = imeMarkedColumns()
        guard cols > 0 else { return nil }
        return IMECompositionGap(rowY: caretView.frame.origin.y,
                                 rowHeight: cellDimension.height,
                                 gapX: caretView.frame.origin.x,
                                 gapWidth: CGFloat(cols) * cellDimension.width)
    }

    /// Draws the terminal with the composing row's tail (everything at or after the
    /// caret column) shifted right by `gap.gapWidth`, leaving an empty gap that the
    /// marked-text overlay occupies. The hot drawing path is reused verbatim via
    /// clipping + translation, so no per-glyph logic is duplicated and normal rows
    /// are unaffected.
    private func drawWithIMEGap(dirtyRect: NSRect, context: CGContext, gap: IMECompositionGap) {
        let cols = terminal.cols
        let compCol = cellDimension.width > 0 ? Int((gap.gapX / cellDimension.width).rounded()) : 0
        // When the composing text plus the existing characters after the caret would
        // overflow the row, wrap them onto the rows below instead of pushing the
        // trailing text off the right edge.
        let tail = imeTail(fromColumn: compCol, cols: cols)
        let tailColumns = tail.reduce(0) { $0 + $1.width }
        if compCol + imeMarkedColumns() + tailColumns > cols {
            drawWrappedComposition(dirtyRect: dirtyRect, context: context, gap: gap, compCol: compCol, cols: cols, tail: tail)
            return
        }
        let bufferOffset = terminal.displayBuffer.yDisp
        let viewWidth = bounds.width

        // Pass 1: the whole view except the composing row's tail region
        // (gapX..width within the composing row's vertical band).
        context.saveGState()
        let belowRow = CGRect(x: 0, y: 0, width: viewWidth, height: gap.rowY)
        let aboveRow = CGRect(x: 0, y: gap.rowY + gap.rowHeight,
                              width: viewWidth,
                              height: max(0, bounds.height - (gap.rowY + gap.rowHeight)))
        let rowLeft = CGRect(x: 0, y: gap.rowY, width: max(0, gap.gapX), height: gap.rowHeight)
        context.clip(to: [belowRow, aboveRow, rowLeft])
        drawTerminalContents(dirtyRect: dirtyRect, context: context, bufferOffset: bufferOffset)
        context.restoreGState()

        // Pass 2: the composing row's tail, shifted right by the gap width.
        // The clip (shifted-tail band) keeps every other row and the line's left
        // portion from being redrawn.
        context.saveGState()
        let shiftedTail = CGRect(x: gap.gapX + gap.gapWidth, y: gap.rowY,
                                 width: max(0, viewWidth - (gap.gapX + gap.gapWidth)),
                                 height: gap.rowHeight)
        context.clip(to: [shiftedTail])
        context.translateBy(x: gap.gapWidth, y: 0)
        drawTerminalContents(dirtyRect: dirtyRect, context: context, bufferOffset: bufferOffset)
        context.restoreGState()

        // Fill the opened gap with the terminal background, then draw the
        // composing (marked) text cell-snapped into it so it lines up with the
        // surrounding monospace grid and the text that will eventually commit.
        context.saveGState()
        let gapRect = CGRect(x: gap.gapX, y: gap.rowY, width: gap.gapWidth, height: gap.rowHeight)
        context.setFillColor(nativeBackgroundColor.cgColor)
        context.fill(gapRect)
        context.restoreGState()

        drawComposingText(in: context, gap: gap)
    }

    /// Cell width (in terminal columns) of a single Character, matching the
    /// terminal's width table (CJK = 2, combining marks = 0, minimum 1).
    private func imeCellWidth(_ ch: Character) -> Int {
        var w = 0
        for scalar in ch.unicodeScalars {
            let cw = UnicodeUtil.columnWidth(rune: scalar)
            if cw > 0 { w += cw }
        }
        return max(1, w)
    }

    /// Draws the composing (marked) text inside the gap, snapping each character
    /// to the cell grid so full-width glyphs occupy two cells exactly like the
    /// committed buffer text. Underlined to mark it as uncommitted.
    private func drawComposingText(in context: CGContext, gap: IMECompositionGap) {
        guard let composing = markedTextStorage?.string, !composing.isEmpty else { return }
        context.saveGState()
        context.textMatrix = .identity
        context.setShouldAntialias(true)
        let yOffset = ceil(CTFontGetDescent(fontSet.normal) + CTFontGetLeading(fontSet.normal))
        let baselineY = gap.rowY + yOffset
        var column = 0
        for ch in composing {
            ...
            let ctLine = CTLineCreateWithAttributedString(NSAttributedString(string: String(ch), attributes: attrs))
            context.textPosition = CGPoint(x: gap.gapX + CGFloat(column) * cellDimension.width, y: baselineY)
            CTLineDraw(ctLine, context)
            column += imeCellWidth(ch)
        }
        // Underline the whole composing region to mark it as uncommitted.
        context.setShouldAntialias(false)
        context.setStrokeColor(nativeForegroundColor.cgColor)
        context.setLineWidth(1.0)
        context.move(to: CGPoint(x: gap.gapX, y: gap.rowY + 1))
        context.addLine(to: CGPoint(x: gap.gapX + gap.gapWidth, y: gap.rowY + 1))
        context.strokePath()
        context.restoreGState()
    }

    /// Lays out the composing (marked) text — and the existing characters after the
    /// caret — starting at the caret, wrapping onto the rows below when they overflow
    /// the line so long mid-line compositions stay fully visible. The rows below the
    /// caret are assumed to be empty (true while editing the last input line); the
    /// trailing text is drawn in the default foreground colour without an underline.
    private func drawWrappedComposition(dirtyRect: NSRect, context: CGContext, gap: IMECompositionGap, compCol: Int, cols: Int, tail: [(ch: Character, width: Int)]) {
        let bufferOffset = terminal.displayBuffer.yDisp
        guard let composing = markedTextStorage?.string, !composing.isEmpty, cols > 0 else {
            drawTerminalContents(dirtyRect: dirtyRect, context: context, bufferOffset: bufferOffset)
            return
        }

        // Normal full draw underneath; the wrapped composition is overdrawn on top.
        drawTerminalContents(dirtyRect: dirtyRect, context: context, bufferOffset: bufferOffset)

        // Visual sequence: composing text (underlined) then the trailing text (plain).
        var sequence: [(ch: Character, width: Int, composing: Bool)] = []
        for ch in composing { sequence.append((ch, imeCellWidth(ch), true)) }
        for t in tail { sequence.append((t.ch, t.width, false)) }

        let cellW = cellDimension.width
        let cellH = gap.rowHeight
        let yOffset = ceil(CTFontGetDescent(fontSet.normal) + CTFontGetLeading(fontSet.normal))

        // Pre-compute how many rows below the caret the sequence wraps onto.
        var wrapRows = 0
        var simCol = compCol
        for item in sequence {
            if simCol + item.width > cols { wrapRows += 1; simCol = 0 }
            simCol += item.width
        }

        context.saveGState()
        // Erase the affected area up front (before any Core Text drawing, which would
        // otherwise leave the fill colour set to the text colour and paint bright bars).
        context.setFillColor(nativeBackgroundColor.cgColor)
        context.fill(CGRect(x: gap.gapX, y: gap.rowY, width: max(0, bounds.width - gap.gapX), height: cellH))
        if wrapRows > 0 {
            for r in 1...wrapRows {
                let y = gap.rowY - CGFloat(r) * cellH
                context.fill(CGRect(x: 0, y: y, width: bounds.width, height: cellH))
            }
        }

        // Draw the sequence, wrapping at the terminal width.
        var col = compCol
        var rowK = 0
        for item in sequence {
            if col + item.width > cols { rowK += 1; col = 0 }
            let x = CGFloat(col) * cellW
            let yTop = gap.rowY - CGFloat(rowK) * cellH
            ...
            let ctLine = CTLineCreateWithAttributedString(NSAttributedString(string: String(item.ch), attributes: attrs))
            context.textPosition = CGPoint(x: x, y: yTop + yOffset)
            CTLineDraw(ctLine, context)
            ...
            if item.composing {
                ...
                context.setStrokeColor(nativeForegroundColor.cgColor)
                context.setLineWidth(1.0)
                context.move(to: CGPoint(x: x, y: yTop + 1))
                context.addLine(to: CGPoint(x: x + CGFloat(item.width) * cellW, y: yTop + 1))
                context.strokePath()
            }
    ...
    /// Extracts the existing characters from `compCol` to the end of the cursor's
    /// buffer line, used to reflow the text after the caret during a wrapped IME
    /// composition. Continuation cells of wide characters are skipped.
    private func imeTail(fromColumn compCol: Int, cols: Int) -> [(ch: Character, width: Int)] {
        var result: [(ch: Character, width: Int)] = []
        let buffer = terminal.buffer
        let absRow = buffer.yBase + buffer.y
        guard compCol >= 0, absRow >= 0, absRow < buffer.lines.count else { return result }
        let line = buffer.lines[absRow]
        var c = compCol
        while c < cols {
            let cd = line[c]
            let w = Int(cd.width)
            if w == 0 { c += 1; continue }   // continuation cell of a wide character
            if cd.code == 0 { break }         // unwritten cell -> end of content
            result.append((terminal.getCharacter(for: cd), max(1, w)))
            c += w
        }
        return result
    }
    ...
    private func updateMarkedTextOverlay() {
        // The marked-text overlay is a subview that updates itself, but the
        // terminal view must also repaint so draw() can open, resize, or close
        // the IME composition gap as the composition changes. Without this the
        // gap logic never runs while composing.
        needsDisplay = true

        // Hide the block caret while composing so it doesn't paint the buffer
        // character at the composition start on top of the composing text. The
        // composing text's underline marks the edit position instead.
        caretView?.isHidden = (markedTextStorage != nil)

        // The composing text is drawn cell-snapped inside the IME gap by
        // drawWithIMEGap on the CoreGraphics path, so the floating NSTextField
        // is only needed when the Metal renderer owns drawing (draw() bypassed).
        var useFloatingOverlay = false
#if canImport(MetalKit)
        useFloatingOverlay = metalView != nil
#endif
        guard useFloatingOverlay, let markedTextStorage, markedTextStorage.length > 0 else {
    ...
    open override func mouseDown(with event: NSEvent) {
        // Warp-style: clicking inside the terminal while an IME composition is in
        // flight commits it instead of losing it. Focus stays in our window, so the
        // click is an intentional in-app action where committing is the right call.
        if let composing = markedTextStorage?.string, !composing.isEmpty {
            markedTextStorage = nil
            markedSelectedRange = NSRange(location: NSNotFound, length: 0)
            kittyIsComposing = false
            send(txt: composing)
